...
namespace ispc
{
    extern "C"
    {
        void build_leaves(int *tree, const int *data, int n);

        void batch_query(int *tree, int n, const int *q_L, const int *q_R, int *results, int count);
    }
}
...
namespace ISPC_Impl
{
    ...
    public:
        ...
        // SIMD-friendly batch query
        void query_batch(const std::vector<int> &Ls, const std::vector<int> &Rs, std::vector<int> &results)
        {
            for (size_t i = 0; i < Ls.size(); ++i)
            {
                results[i] = (int)query(Ls[i], Rs[i]);
            }
        }
    };
}
